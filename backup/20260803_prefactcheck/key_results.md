...
## 14. All-drop main table — baseline backfill (2026-07-28/29 overnight window)

> Protocol: AutoVLA-3B, `prune_variant=drop` (true token removal). Baselines = **raw**,
> no denylist. **(ours)** = **+fallback** (denylist->no-prune, 768 tokens; reproduces
> 0.9045 exactly). No-prune ref = 0.89879. Candidate table:
> `docs/results/2026-07-29_physical_drop_main_table.md`. Generator: `scripts/gen_table1_draft.py`.

### 14.1 Baseline drop — shard0 (N=2949) placeholders
| method | r=0.75 | r=0.5 | r=0.25 |
|---|---:|---:|---:|
| FastV (fastv_l2) | 0.8736 | 0.8191 | 0.6783 |
| Random | 0.8731 | 0.8635† | 0.7670 |
| PruMerge (prumerge_cls) | 0.8511 | 0.7887 | 0.6511 |
| SparseVLM (sparsevlm_text) | 0.8991† | 0.8774 | 0.8293 |

† full navtest (N=11576): `MT_random_r05`=0.86349, `MT_sparsevlm_text_r075`=0.89908.
All other baseline cells are shard0-only (drop) and **not yet full navtest**.

### 14.2 Ours (full navtest, drop, +fallback unless noted)
| method | r=0.75 | r=0.5 | r=0.25 | dynamic |
|---|---:|---:|---:|---:|
| SFT Scorer | 0.8946 | **0.9045** | (not run) | — |
| RL Scorer | 0.8944 | 0.8928 | 0.8264 | — |
| SFT + τ-cut (MSE kr060) | — | — | — | 0.8949 |
| Budget RL | — | — | — | 0.8701 |

RL r=0.75 completed to full navtest this window (`MT_rl_shaped_r075_sh0-3`, +fb 0.8944).

### 14.3 Headline & caveats
- At r=0.5, **SFT (ours) 0.9045 > all baselines** (SparseVLM 0.877 / FastV 0.819 /
  PruMerge 0.789) and ≈ no-prune (100.6%). Gap widens at r=0.25.
- **Not done:** baseline full-shard upgrade (Phase B2 deadline-skipped by a too-high
  headroom gate `FULLSH_NEED=13000s` vs 9618s left at 11:04); SFT r=0.25 (non-headline);
  7B/nuScenes (image tars name-mismatch — invalid, see BESTEFFORT_REPORT.md).
- FastV old CSVs `MT_fastv_l2_r0{5,75}` are **mask** (not drop); use `MT_fastv_l2_drop_*`.
- Changelog: 2026-07-29 added §14 (baseline drop shard0 + RL r=0.75 full + ours summary).
