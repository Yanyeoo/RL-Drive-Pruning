...
### Revision 2026-07-03: 重启动态 token 剪枝为主线目标（撤销 07-01 的"路 A 退守"）

**触发**

07-01 在 offline dynamic head-gating probe 天花板（K_eff=4≈静态 Sc4）被证伪后，framing 曾退守到"路 A：层级可剪性判据分析论文"。user 2026-07-03 明确决定**坚持动态剪枝并实现原始目标**（design doc Q1–Q3 的自适应 token 剪枝 Pareto dominate 静态）。

**关键澄清（事实核验，2026-07-03 code-explorer 全仓盘点）**

被 06-30/07-01 数据证伪的是 **L12 attention-head gating 的 offline probe**（16-head 窄动作空间，surrogate loss 把它锚死在"删 bot-4"≈静态）。而 Q1–Q4 设计的**真正动态方法 = ViT→LLM 接口的 vision token 级剪枝（N≈720 token 里按 importance 选 top-B）+ Budget Policy，online GRPO 训**——**这套代码从未搭建、从未测过，不在被证伪范围内**。故原始目标仍然开放。

**现有地基 / 缺口（决定本 Revision 的执行顺序）**
- 已具备：非侵入 attention 捕获（`PromptIndex` 已能定位 720 个 vision token 位置）、o_proj/forward hook 注入范式、`PerHeadBinaryProbe` 轻量 scorer 训练范式、上游 `GRPOAutoVLA`+真实 NAVSIM PDMS reward 骨架、全量 navtrain per-token attention dump `(28,16,720)`（可离线做 token-level label）。
- 缺失（需新建）：①vision **token 级**剪枝执行器；②per-**token** Importance Scorer；③Budget Policy（4-class）；④剪枝决策接入 online GRPO rollout。

**决策：分阶段关键路径，S2 为"决定性 gate"，先测再大建**

| 阶段 | 内容 | 依赖 | gate |
|---|---|---|---|
| **S1** | token 剪枝执行器（先 attention-mask 变体，安全无 position surgery）+ r=1.0 lossless sanity | 纯代码 | 通过 = r=1.0 复现 B0(0.8983) 位级一致 |
| **S2** | fixed-r Pareto {0.25/.5/.75/1.0} + per-scene oracle r*，**selector = L12 attention（免训练）** | S1 | **通过 = 存在 headroom 且最优 r 有显著 scene 方差；否则动态无意义，回报 user** |
| **S3** | token Importance Scorer(LambdaRank，label 来自 dump) → Budget Policy → online GRPO | S2 pass | 完整动态方法，产 M6 主表 |

**理由**：Q1-B 底线 + claim② "自适应>固定" 的成立前提是 token 剪枝有可剪空间且 scene-adaptive。该前提（R-D-3）**从未为 token 剪枝测过**。S1+S2 便宜、决定性、几乎全复用现有资产；只有 S2 gate 通过才投入 S3 的重型 RL。这样既坚持动态目标，又避免盲目烧数周。

**详细技术方案**：`docs/specs/dynamic_token_pruning_S1_spec.md`（执行器）、`docs/specs/dynamic_headroom_gate_S2_spec.md`（gate 实验）。

**与旧决策的关系**：Q1–Q5 主设计**不变**（本 Revision 是"恢复执行原设计"，非改设计）。路 A 的"层×可剪性 landscape"（key_results §6.8）降级为**支撑性分析 / ablation A5 证据**，不再是论文主线。路 C（online real-reward dynamic）由"future work"**升为主线**。

**Reject 的方案**
- **继续路 A（判据分析论文）**：user 明确否决；且 landscape 已证 bot-4 head 删除近乎处处 free → 静态故事本身"可剪空间小"，efficiency headline 弱。
- **直接全建 S3 跳过 S2 gate**：违反事实优先原则，headroom 未知即投重型 RL 风险高（已在 q1 供选，user 选择先出文档评审）。
- **复用 head-gating dynamic mask agent 当动态方法**：它是 per-scene 常量 **head** 掩码（EVAL 路径），非 token 级、非可训练策略，且天花板已证伪。

---

### Revision 2026-07-06: 撤销"动态预算(Budget Policy)"，方法定为"动态 token 选择 + 固定 r=0.5"（user 拍板 A）

**触发**：S3 Phase A（W1）实测 claim②（自适应预算 > 固定比例）为**负**（key_results §11）。6 种配置（budget 输入从运动学 ctx → 分数摘要 → 完整 720 分数曲线；分类/回归两目标）**全部跑不赢固定 r=0.5**（最好 −0.23pt，且更耗算力）。诊断：PDMS 确定性 → per-scene 最优比例的 oracle 增益(+1.6~2.2pt)真实，但**不是任何可观测特征（含 scorer 自身完整重要性曲线）的可学函数**。

**决策（user 选 A，2026-07-06）**：
- **方法 = 驾驶条件化 token Importance Scorer（动态选"mask 哪些 token"）+ 固定 keep-ratio r=0.5**。
- **撤销 Budget Policy（Stage B）与其 online GRPO**（原 Q3.b 4-class budget、Q4.2 oracle-SFT、Q4.3.b R3 reward、M3/M4/M5）——降为**论文中的诚实负面结果 / ablation**（"per-scene 动态预算相对固定比例无增益，oracle 余量不可学"）。
- **claim② 由主张改为负面结论**；claim①（≥50% 省算力近乎无损）与 claim③（driving-conditioned scorer ≫ vision-attention/random）为论文正面主线。

**对 Q1–Q5 的影响**：
- Q1：A 主轴(≥50%省算力)/B 底线(不掉点) 不变；C novelty 从"scene-adaptive budget + RL budget policy"收窄为"**driving-conditioned token importance（固定比例）**"。
- Q3.b（4-class Budget Policy）、Q4.2（budget oracle）、Q4.3.b（Stage B reward）、M4/M5：**停用**（保留代码/文档作负面结果依据）。
- Q3.a/Q4.1（Importance Scorer, LambdaRank SFT）、Q5（评测/baselines/Pareto）：**保留**，为主线。
- Scorer 的 online GRPO（M2）：**可选/低优先**（SFT scorer 已胜 attn_L12；§10）。

**未 close 的唯一动态比例机制（记录，暂不做）**：标定分数 + 全局阈值 τ 的"涌现动态比例"（连续比例，需 GPU 评测）。user 选 A 暂不测；若日后想彻底 close 再做。reverse：见 s3_execution_plan.md。

**下一步**：全 navtest 主表（scorer r∈{.25,.5,.75} + attn_L12/random/r1.0 × 4 shard，运行中）→ 聚合出固定 r=0.5 iso-compute 主表 + Pareto → M6（补 FastV / FastV-selector-at-input baselines + 消融 + 图）。
