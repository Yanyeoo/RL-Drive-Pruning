    ...
    for epoch in range(args.num_epochs):
        ...
        for g_start in range(0, len(valid_scenes), args.group_size):
            ...
            for idx in perm[g_start:g_start + args.group_size]:
                ...
                # v5: Use true PDMS product as reward (same scoring as navtest eval).
                # This eliminates the simulator-proxy → shaped-delta reward gap
                # that caused v3/v4 training reward to not correlate with eval PDMS.
                # Efficiency is NOT added to reward — the budget head learns
                # the optimal kr from pure driving quality signal.
                ...
                # Safety penalty: penalize degradation on safety-critical sub-metrics
                # relative to the unpruned baseline (still needed as a guardrail).
                ...
                # v5: NO efficiency bonus in reward.
                # v3/v4 evidence: efficiency_bonus pushed kr lower without improving
                # PDMS, causing BEST(0.879) > FINAL(0.861). The budget head learns
                # optimal kr from pure driving reward signal.
                total_reward = (
                    driving_reward
