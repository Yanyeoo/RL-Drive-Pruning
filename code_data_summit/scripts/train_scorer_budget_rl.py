...
def main():
    ...
    if is_main:
        print("=" * 70, flush=True)
        print("[budget-rl] Safe-HTPO-Lite — joint WHAT + HOW MANY policy", flush=True)
        print("=" * 70, flush=True)
        print(f"  data            = {args.json_dir}")
        print(f"  world_size      = {world_size} (synchronized gradients)")
    ...
    for epoch in range(args.num_epochs):
        ...
        for g_start in range(0, len(valid_scenes), args.group_size):
            ...
            for idx in perm[g_start:g_start + args.group_size]:
                ...
                # The full-token pass that captured features also supplies the
                # same-scene unpruned trajectory. Score it locally so navtrain
                # delta and safety rewards never depend on navtest artifacts.
                baseline_out = reward_fn.rl_pdm_score(
                    result["baseline_trajectory"], result["token"],
                    shaped=True, baseline_scores=None, return_components=True,
                )
                if not isinstance(baseline_out, tuple):
                    continue
                _, baseline = baseline_out
                required = {"collision", "drivable", "progress", "ttc", "comfort", "direction"}
                if not required.issubset(baseline):
                    continue

                # Score the pruned trajectory against this exact same-scene
                # unpruned baseline for both delta and safety terms.
                reward_out = reward_fn.rl_pdm_score(
                    result["trajectory"], result["token"], shaped=True,
                    baseline_scores=baseline, return_components=True,
                )
                if not isinstance(reward_out, tuple):
                    continue
                driving_reward, sub_scores = reward_out
                if not required.issubset(sub_scores):
                    continue
                driving_reward = args.driving_scale * driving_reward

                safety_loss = sum(
                    weight * max(0.0, baseline[name] - sub_scores[name] - args.safety_margin)
                    for name, weight in {"collision": 0.40, "drivable": 0.30, "ttc": 0.30}.items()
                )

                # Efficiency still rewards token reduction, but never fixes a
                # per-scene ratio; safety loss counteracts unsafe over-pruning.
                efficiency_bonus = 1.0 - result["keep_ratio"]
                total_reward = (
                    driving_reward
                    - args.safety_beta * safety_loss
                    + args.efficiency_beta * efficiency_bonus
                )
            ...
            if len(group_rewards) >= 2:
                ...
                # The same normalized reward updates the continuous budget and
                # the selected Top-B token set.
...
def _save(model, feat_mean, feat_std, n_cam, budget_log_std, out_dir, tag, args):
    ...
    (save_dir / "manifest.json").write_text(json.dumps({
        "spec": "budget_rl_v3_joint_selection_budget",
        "method": "Joint REINFORCE over Top-K token selection and Gaussian budget; driving reward (scaled) + efficiency bonus; KL on token_net only",
