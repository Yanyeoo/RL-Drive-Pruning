#!/usr/bin/env bash
# auto_chain_v5.sh — v5 无人值守全链脚本
#
# 链结构:
#   wait_train → v5 full eval (4GPU, 4shard, ~3.6h)
#   → ablation (ckpt_best / safety_net / SFT r=0.355 / v3对比)
#   → 7B nuScenes cross-model eval (best-effort)
#   → 汇总报告
#
# 用法: bash scripts/auto_chain_v5.sh
# 前提: v5 训练已启动 (PIDs 在 logs/v5_train/train.pids)
set -uo pipefail
...
CHAINLOG="$ROOT/logs/v5_train/auto_chain_$(date +%Y%m%d_%H%M%S).log"
LOGDIR="$ROOT/logs/auto_chain_v5"
mkdir -p "$LOGDIR"
...
###############################################################################
# Step 0: Wait for v5 training
###############################################################################
log "=== Step 0: Waiting for v5 training ==="
TRAIN_PIDFILE="$ROOT/logs/v5_train/train.pids"
OUTBASE_FILE="$ROOT/logs/v5_train/outbase"
...
V5_SH0="${OUT_BASE}_sh0"
V5_BEST="${OUT_BASE}_sh0/ckpt_best"
log "v5 ckpt: $V5_SH0"

if [[ ! -f "$V5_SH0/checkpoint.pt" ]]; then
  LATEST=$(ls -d "${OUT_BASE}_sh0/ckpt_step"* 2>/dev/null | sort -V | tail -1)
  if [[ -n "$LATEST" && -f "$LATEST/checkpoint.pt" ]]; then
    V5_SH0="$LATEST"
    log "v5 fallback to latest step: $V5_SH0"
  else
    log "FATAL: no v5 checkpoint found"
    exit 2
  fi
fi

###############################################################################
# Step 1: v5 FINAL full eval (4 GPU, 4 shard, ~3.6h)
###############################################################################
log "=== Step 1: v5 FINAL full eval (4 GPU x 4 shard) ==="
TAG_V5F="SOTAV5_R1_FINAL"
for SH in 0 1 2 3; do
  eval_shard $SH "$V5_SH0" "$TAG_V5F" $SH &
done
wait
log "v5 eval done at $(date)"
aggregate_csvs "$TAG_V5F" 2>&1 | tee -a "$CHAINLOG"

###############################################################################
# Step 2: v5 ckpt_best eval + 消融
###############################################################################
log "=== Step 2: Ablation experiments ==="

# 2a: v5 ckpt_best eval (GPU0)
if [[ -f "$V5_BEST/checkpoint.pt" ]]; then
  eval_shard 0 "$V5_BEST" "SOTAV5_R1_BEST" 0 &
  PID_BEST=$!
  log "GPU0 START v5 BEST"
else
  log "SKIP v5 BEST: no ckpt_best"
  PID_BEST=""
fi

# 2b: v5 FINAL + safety_net (GPU1)
eval_shard 1 "$V5_SH0" "SOTAV5_R1_SAFENET" 0 "+agent.safety_net=true" &
PID_SAFE=$!
log "GPU1 START v5 SAFENET"
...
# 2d: v3 old ckpt shard0 — v3 vs v5 直接对比 (GPU3)
...
aggregate_csvs "SOTAV5_R1_BEST" 2>&1 | tee -a "$CHAINLOG"
aggregate_csvs "SOTAV5_R1_SAFENET" 2>&1 | tee -a "$CHAINLOG"
...
all_kr = {}
for f in sorted(glob.glob("/apdcephfs/private_shayladeng/tokenrl_autoVLA/logs/auto_chain_v5/_SOTAV5_R1_FINAL_sh*.log")):
...
if vals_all:
    ...
    with open("/apdcephfs/private_shayladeng/tokenrl_autoVLA/logs/auto_chain_v5/kr_distribution.json", "w") as f:
...
for sh in range(4):
    pattern = f"/apdcephfs/private_shayladeng/tokenrl_autoVLA/ckpt/s3_token_scorer_budget_rl_v5_*_sh{sh}/train_log.jsonl"
...
for tag in ["SOTAV5_R1_FINAL", "SOTAV5_R1_BEST", "SOTAV5_R1_SAFENET",
...
# Comparison
v5_pdms = report["results"].get("SOTAV5_R1_FINAL", {}).get("PDMS", None)
print("\n=== KEY COMPARISON ===")
for name, pdms in baselines.items():
    print(f"  {name}: {pdms:.5f}")
if v5_pdms:
    print(f"  v5 FINAL: {v5_pdms:.5f}")
    delta_sft = v5_pdms - 0.89008
    delta_v4 = v5_pdms - 0.86094
    print(f"  v5 vs SFT r=0.5: {delta_sft:+.5f}")
    print(f"  v5 vs v4:         {delta_v4:+.5f}")
    if v5_pdms > 0.89008:
        print("  >>> v5 EXCEEDS SFT baseline! ICLR core claim valid.")
    elif v5_pdms > 0.885:
        print("  >>> v5 close to SFT, promising direction.")
    else:
        print("  >>> v5 below SFT. Need per-token credit assignment (ICLR PLAN §2).")

with open("/apdcephfs/private_shayladeng/tokenrl_autoVLA/logs/auto_chain_v5/final_report.json", "w") as f:
    json.dump(report, f, indent=2)

report["baselines"] = baselines
print(f"\n  JSON report: logs/auto_chain_v5/final_report.json")
PYEOF
...
log "Report: logs/auto_chain_v5/final_report.json"
log "CSVs:   results/raw/SOTAV5_R1_*.csv"
log "Logs:   logs/auto_chain_v5/"
