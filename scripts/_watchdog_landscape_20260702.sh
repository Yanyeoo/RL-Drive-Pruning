#!/usr/bin/env bash
# ============================================================================
# _watchdog_landscape_20260702.sh — long-lived watchdog for cycle ending
# 2026-07-03 18:00. Every LOOP_SLEEP seconds until deadline (or STOP_DRIVER):
#   - refresh the landscape + magnitude figures
#   - if the driver is dead, queue is unfinished, GPUs are idle, and we are
#     before the deadline -> relaunch the 20260702 driver (SKIP_DONE-safe).
# Never double-launches (checks pgrep first). Never rm/mv anything.
#
# Launch: nohup bash scripts/_watchdog_landscape_20260702.sh > logs/_watchdog_landscape_20260702.log 2>&1 &
...
while true; do
  ...
  driver_up=$(pgrep -f '_driver_landscape_20260702.sh' >/dev/null && echo 1 || echo 0)
  ...
  if [[ "$driver_up" == "0" ]] && [[ "$gpu_busy" == "0" ]] && queue_unfinished; then
    ...
    nohup bash scripts/_driver_landscape_20260702.sh \
