#!/usr/bin/env bash
# ============================================================================
# _driver_landscape_20260701.sh — unattended path-A landscape backfill driver
# ----------------------------------------------------------------------------
# Reads the variant queue from landscape_queue.txt (re-read each iteration, so
# it can be extended live). Runs one variant at a time on 2× H20 (no double-
# open). Before each variant, safely injects its bot-K mask into the sweep
# script (only between dispatchers, when no sweep process runs). After each
# variant lands: harvest PDMS, refresh both figures, cp -a backup.
#
# Stops on: STOP sentinel OR deadline 2026-07-02 18:00 OR queue exhausted.
# SKIP_DONE=1 => safe to restart.
# Launch: nohup bash scripts/_driver_landscape_20260701.sh > logs/_driver_landscape.log 2>&1 &
...
# 1) wait for any pre-existing dispatcher (in-flight L24K4) to finish
