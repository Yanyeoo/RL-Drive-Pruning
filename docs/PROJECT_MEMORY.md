...
> 写给下一个 AI / 协作者。最后更新: 2026-08-14。
> ICLR 2027 ddl: 2026-09-25。

---

## ★★★ 核心目标（2026-08-14 用户明确指令，最高优先级）★★★

> **不可更改的总目标：在当前 RL idea 无大改的情况下达到 SOTA 且具备 efficiency。** 这不是"防守性补数据"，是当前唯一主线；后续 AI 不得降低、替换或改写该目标。

| 指标 | 当前值 | 目标 |
|---|---|---|
| SFT scorer r=0.5（LambdaRank，最强 learned 基线） | 0.89199 (full N=11576) | — |
| no-prune r=1.0（理论上界） | 0.89879 | — |
| **best RL（shaped reward r=0.5）** | 0.89094（**−0.105 vs SFT**） | — |
| **RL 目标（分两步）** | — | **① 先超 SFT（>0.892）② 逼近/超越 no-prune（→0.899）** |

**RL 失败现状（v3→v6 全线退化）**：0.8707 → 0.8609 → 0.8485 → 0.8787，没有一版超过 SFT。

**根因 + 改进方向**：见 `docs/journal/2026-08-14_rl_to_sota.md`（4 条根因 + 5 个方向 + 推荐组合拳）。
